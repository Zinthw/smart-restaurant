/**
 * QR Controller
 * Handles QR code generation, download and verification
 */
  ...
  try {
    ...
    if (!tableId || !token) {
      return res.status(400).json({
        valid: false,
        message: "Missing tableId or token",
      });
    }
    ...
    let decoded;
    try {
      decoded = jwt.verify(token, process.env.QR_JWT_SECRET || "qr_secret_key");
    } catch (err) {
      return res.json({
        valid: false,
    ...
    // Check if tableId in token matches
    if (decoded.tableId !== tableId && decoded.tableId !== parseInt(tableId)) {
      return res.json({
        valid: false,
        message: "Token không khớp với bàn",
      });
    }

    // Get table info from database
    const { rows } = await db.query(
      "SELECT id, table_number, capacity, location, status FROM tables WHERE id = $1",
      [tableId]
    );
  ...
  try {
    ...
    if (tables.length === 0) {
      return res
        ...
        .json({
          message:
            "Không có bàn nào đang hoạt động (active) hoặc chưa tạo QR để tải.",
        });
    }
    ...
    if (format === "png") {
      ...
      res.setHeader(
        "Content-Disposition",
        'attachment; filename="all-qrs.zip"'
      );
      ...
      res.setHeader(
        "Content-Disposition",
        'attachment; filename="all-qrs.pdf"'
      );
      ...
      return res
        .status(400)
        .json({ message: "Invalid format. Use ?format=png or ?format=pdf" });
    }
  } catch (err) {
    next(err);
  }
...
exports.download = async (req, res, next) => {
  try {
    ...
    if (!table || !table.qr_token) {
      return res
        .status(404)
        .json({ error: "NOT_FOUND", message: "Table or QR not found" });
    }
    ...
    if (format === "png") {
      ...
      QRCode.toBuffer(url, opts, (err, buffer) => {
        ...
        res.setHeader(
          "Content-Disposition",
          `attachment; filename="table-${table.table_number}.png"`
        );
        res.send(buffer);
      });
    } else if (format === "pdf") {
      ...
      return res
        .status(400)
        .json({ error: "VALIDATION_ERROR", message: "Invalid format" });
    }
  } catch (err) {
    next(err);
  }
 ...
 */
exports.regenerateAll = async (req, res, next) => {
  try {
    // Lấy tất cả các bàn đang hoạt động
    const { rows: tables } = await db.query(
      "SELECT * FROM tables WHERE status = 'active'"
    );

    if (tables.length === 0) {
      return res
        .status(400)
        .json({ message: "No active tables found to regenerate." });
    }

...
export default function TableFormModal({ open, onClose, onSubmit, initialData }) {
  ...
  useEffect(() => {
    ...
    else reset({ tableNumber: '', capacity: 2, location: 'Indoor', description: '' });
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-content" onClick={e => e.stopPropagation()}>
        <div style={{ padding: '20px', borderBottom: '1px solid #eee', display: 'flex', justifyContent: 'space-between' }}>
          <h3 style={{ margin: 0 }}>{initialData ? 'Edit Table' : 'Add New Table'}</h3>
        ...
        <form onSubmit={handleSubmit(onSubmit)} style={{ padding: '20px' }}>
          <div className="form-group">
            ...
            <input className="form-input" placeholder="e.g. T-01" {...register('tableNumber', { required: true })} />
            {errors.tableNumber && <span className="form-hint error">Required</span>}
          </div>

          <div style={{ display: 'flex', gap: '15px' }}>
            <div className="form-group" style={{ flex: 1 }}>
              <label className="form-label">Capacity</label>
              <input type="number" className="form-input" {...register('capacity', { required: true, min: 1, max: 20 })} />
            </div>
            <div className="form-group" style={{ flex: 1 }}>
          ...
          <div style={{ display: 'flex', gap: '10px', marginTop: '20px' }}>
            <button type="submit" className="btn-primary" style={{ flex: 1 }}>Save Table</button>

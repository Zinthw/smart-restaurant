...
import { getTables, createTable, updateTable, updateTableStatus, generateQR } from '../../api/tables.api';
...
export default function TableList() {
  ...
  const handleSave = async (data) => {
    try {
      ...
      setShowForm(false); setSelectedTable(null); loadTables();
    } catch (error) { toast.error("Error saving table"); }
  };

  const handleToggleStatus = async (table) => {
    try {
      await updateTableStatus(table.id, table.status === 'active' ? 'inactive' : 'active');
  ...
  const handleGenerateQR = async (table) => {
    try {
      ...
      if (url) { setQrUrl(url); setSelectedTable(table); setShowQR(true); toast.success("QR Generated!"); }
    } catch (error) { toast.error("Error generating QR"); }
  };
  ...
  return (
    <AdminLayout>
      ...
      <div className="admin-header">
        ...
        <button className="btn-primary" onClick={() => { setSelectedTable(null); setShowForm(true); }}>
          + Add Table
        </button>
      </div>
      ...
      <div className="table-card">
        <div className="table-header">
          <h3>All Tables</h3>
          <button className="btn-secondary">⬇️ Download All QR</button>
        </div>
        ...
        {loading ? <div className="p-5 text-center">Loading...</div> : (
          <div className="tables-grid">
            {tables.map(t => (
              <div 
                key={t.id} 
                className={`table-tile ${t.status === 'active' ? 'available' : 'inactive'}`}
              >
                ...
                <div className="table-actions">
                  <button className="btn-small" onClick={() => handleGenerateQR(t)} title="QR Code">
                   QR
                  </button>
                  <button className="btn-small" onClick={() => { setSelectedTable(t); setShowForm(true); }} title="Edit">
                    ✏️
                  </button>
                  <button className="btn-small" onClick={() => setConfirm({
                      table: t,
      ...
      {/* Modals */}
      <TableFormModal open={showForm} onClose={() => setShowForm(false)} onSubmit={handleSave} initialData={selectedTable} />
      <QRModal open={showQR} onClose={() => setShowQR(false)} table={selectedTable} qrUrl={qrUrl} />
      <ConfirmDialog open={!!confirm} title="Confirm Action" message={confirm?.message} onConfirm={() => handleToggleStatus(confirm.table)} onCancel={() => setConfirm(null)} />
    </AdminLayout>
  );
}
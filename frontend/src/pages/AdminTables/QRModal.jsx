...
export default function QRModal({ open, onClose, table, qrUrl, onRefresh }) {
  ...
  const handleRegenerate = async () => {
    ...
    const isConfirm = window.confirm(
      `CẢNH BÁO: Bạn có chắc muốn tạo lại mã QR cho bàn ${table.tableNumber}?\nMã QR cũ đang in sẽ KHÔNG còn dùng được nữa!`
    );
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-content" style={{ maxWidth: '750px' }} onClick={e => e.stopPropagation()}>
        ...
        <div className="table-header">
          <h3>QR Code Preview - {table?.tableNumber}</h3>
          ...
          <div style={{ display: 'flex', gap: '10px' }}>
             ...
             {/* Nút Close */}
             <button onClick={onClose} className="btn-secondary" style={{ width: '80px', minWidth: 'unset' }}>
                Close
             </button>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', gap: '30px', padding: '30px', flexWrap: 'wrap' }}>
          ...
          <div className="qr-details" style={{ flex: 1.5 }}>
             ...
             <div className="detail-row">
                ...
                <span className="detail-value" style={{color: 'green'}}>Active</span>
             </div>

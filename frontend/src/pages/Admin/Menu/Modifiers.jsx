...
export default function ModifiersPage() {
  ...
  const [formData, setFormData] = useState({ id: null, name: '', required: false, selection_type: 'single' });
  
  // State cho Option Form
  const [optionData, setOptionData] = useState({ id: null, name: '', price: 0 });
  ...
  const fetchGroups = async () => {
    ...
    try {
      ...
      setGroups([
        { 
          id: 1, name: 'Size (Kích cỡ)', selection_type: 'single', 
          options: [{id: 101, name: 'M', price: 0}, {id: 102, name: 'L', price: 5000}, {id: 103, name: 'XL', price: 10000}] 
        },
        { 
          id: 2, name: 'Mức Đường', selection_type: 'single', 
          options: [{id: 201, name: '100%', price: 0}, {id: 202, name: '50%', price: 0}, {id: 203, name: '0%', price: 0}] 
        },
        { 
          id: 3, name: 'Topping (Thêm)', selection_type: 'multi', 
          options: [{id: 301, name: 'Trân châu đen', price: 5000}, {id: 302, name: 'Pudding trứng', price: 5000}, {id: 303, name: 'Thạch dừa', price: 5000}] 
        },
        { 
          id: 4, name: 'Mức Đá', selection_type: 'single', 
          options: [{id: 401, name: '100%', price: 0}, {id: 402, name: '50%', price: 0}, {id: 403, name: 'Không đá', price: 0}] 
        },
        { 
          id: 5, name: 'Loại Vỏ Bánh', selection_type: 'single', 
          options: [{id: 501, name: 'Mỏng giòn', price: 0}, {id: 502, name: 'Dày xốp', price: 0}, {id: 503, name: 'Viền phô mai', price: 15000}] 
        },
        { 
          id: 6, name: 'Sốt Ăn Kèm', selection_type: 'multi', 
          options: [{id: 601, name: 'Tương ớt', price: 0}, {id: 602, name: 'Mayonnaise', price: 0}, {id: 603, name: 'Sốt BBQ', price: 5000}] 
        }
      ]);
    } finally {
      setLoading(false);
  ...
  const handleOpenCreateGroup = () => {
    setFormData({ id: null, name: '', required: false, selection_type: 'single' });
  ...
  const handleSaveGroup = async (e) => {
    e.preventDefault();
    if (!formData.name.trim()) { toast.error("Tên nhóm không được để trống!"); return; }
  ...
  const handleSaveOption = async (e) => {
    e.preventDefault();
    if (!optionData.name.trim()) { toast.error("Tên lựa chọn trống!"); return; }
    if (optionData.price < 0) { toast.error("Giá không được âm!"); return; }
    ...
    const updatedGroups = groups.map(g => {
      if (g.id === selectedGroup.id) {
        ...
        if (isEditOptionMode) {
          ...
          if (optionData.id) {
             newOptions = newOptions.map(opt => opt.id === optionData.id ? optionData : opt);
          } else {
    ...
    toast.success(isEditOptionMode ? "Cập nhật lựa chọn!" : "Thêm lựa chọn thành công!");
  };

  const handleDeleteOption = (group, optionId) => {
    ...
    // Cập nhật lại UI trong modal nếu đang mở
    if (showGroupModal && selectedGroup) {
      const updatedGroup = updatedGroups.find(g => g.id === selectedGroup.id);
      setSelectedGroup(updatedGroup);
    }
    ...
    <>
            ...
            {currentGroups.length > 0 ? (
              currentGroups.map((group) => (
                <tr key={group.id}>
                  ...
                  <td>
                    <span className={`status-badge ${group.selection_type === 'single' ? 'active' : 'inactive'}`}>
                      {group.selection_type === 'single' ? 'Chọn 1 (Radio)' : 'Chọn nhiều (Check)'}
                    </span>
                  </td>
                  <td>
                    {group.options && group.options.length > 0 ? (
                      <div style={{display:'flex', gap: 5, flexWrap:'wrap'}}>
                        {group.options.map((opt, idx) => (
                          <span 
                            key={idx} 
                            style={{background:'#f1f2f6', padding:'2px 8px', borderRadius:4, fontSize:12, border:'1px solid #d1d5db', cursor:'pointer'}}
                            title="Bấm để sửa nhanh"
                            onClick={() => handleOpenEditOption(group, opt, idx)}
                          >
                      ...
                      <span style={{color:'#95a5a6', fontSize:12}}>Chưa có lựa chọn</span>
                    )}
                  </td>
              ...
              <tr><td colSpan="4" style={{textAlign:'center', padding:20}}>Chưa có nhóm nào</td></tr>
            )}
          </tbody>
        </table>
      ...
      {showGroupModal && (
        <div className="modal-overlay" onClick={() => setShowGroupModal(false)}>
          <div className="modal-content" onClick={e => e.stopPropagation()} style={{padding: 30, maxWidth: 600}}>
            ...
            <form onSubmit={handleSaveGroup}>
              ...
              {isEditGroupMode && selectedGroup && (
                <div style={{marginTop: 10, marginBottom: 20, background: '#f8f9fa', padding: 15, borderRadius: 8}}>
                  <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom: 10}}>
                    <h4 style={{margin:0, fontSize:15}}>Danh sách lựa chọn</h4>
                    <button type="button" className="btn-small" onClick={() => handleOpenAddOption(selectedGroup)}>+ Thêm</button>
                  </div>
                  ...
                  <div style={{maxHeight: 200, overflowY:'auto'}}>
                    {selectedGroup.options && selectedGroup.options.length > 0 ? (
                        ...
                        <tbody>
                          {selectedGroup.options.map((opt, idx) => (
                            <tr key={idx} style={{borderBottom:'1px solid #eee'}}>
                              ...
                              <td style={{padding: '8px 0'}}>+{opt.price.toLocaleString()}đ</td>
                              <td style={{textAlign:'right'}}>
                                <button 
                                  type="button" 
                                  style={{border:'none', background:'none', cursor:'pointer', color:'#2980b9', marginRight:10}}
                                  onClick={() => handleOpenEditOption(selectedGroup, opt, idx)}
                                >
                                  Sửa
                                </button>
                                <button 
                                  ...
                                  onClick={() => handleDeleteOption(selectedGroup, opt.id)}
                                >
                                  Xóa
                                </button>
                              </td>
                      ...
                      <div style={{textAlign:'center', color:'#999', fontSize:13, padding: 10}}>Chưa có lựa chọn nào</div>
                    )}
                  </div>
                </div>
              )}
              ...
              <div style={{display:'flex', gap:10, marginTop:20}}>
                <button type="button" className="btn-secondary" style={{flex:1}} onClick={() => setShowGroupModal(false)}>Hủy</button>
                <button className="btn-primary" style={{flex:1}}>Lưu Thay Đổi</button>
              </div>
            </form>
          </div>
        </div>
      ...
      {showOptionModal && selectedGroup && (
        <div className="modal-overlay" style={{zIndex: 1100}} onClick={() => setShowOptionModal(false)}>
          <div className="modal-content" onClick={e => e.stopPropagation()} style={{padding: 30, maxWidth: 400}}>
            ...
            <form onSubmit={handleSaveOption}>
              ...
              <div className="form-group">
                ...
                <input 
                  ...
                  onChange={e => setOptionData({...optionData, price: parseInt(e.target.value) || 0})} 
                />
              </div>
              <div style={{display:'flex', gap:10, marginTop:20}}>
                <button type="button" className="btn-secondary" style={{flex:1}} onClick={() => setShowOptionModal(false)}>Hủy</button>
                <button className="btn-primary" style={{flex:1}}>Lưu</button>
              </div>
            </form>
          </div>
        </div>

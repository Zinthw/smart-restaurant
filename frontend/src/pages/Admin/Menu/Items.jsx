...
export default function ItemsPage() {
    ...
    const initData = async () => {
      ...
      try {
        const [itemsRes, catsRes, modsRes] = await Promise.all([
          ...
          menuApi.getModifierGroups() // Lấy thêm list modifier groups
        ]);
        ...
        setItems([
          ...
          { 
            id: 2, name: 'Salad Cá Ngừ', price: 85000, category_id: 1, status: 'active', order_count: 45,
            is_recommended: false,
            photos: [],
            modifier_group_ids: []
          },
          { 
            id: 3, name: 'Trà Sữa Trân Châu', price: 45000, category_id: 3, status: 'sold_out', order_count: 500,
            is_recommended: true,
            photos: [],
  ...
  // --- LOGIC LỌC & SORT ---
  useEffect(() => setCurrentPage(1), [searchTerm, filterCategory, filterStatus, sortBy]);
  ...
  const getProcessedItems = () => {
    let result = items.filter(item => {
      ...
      if (filterStatus !== 'All') {
         if (filterStatus === 'active') matchStatus = ['active', 'available'].includes(item.status);
         else if (filterStatus === 'sold_out') matchStatus = item.status === 'sold_out';
         else if (filterStatus === 'inactive') matchStatus = ['inactive', 'unavailable'].includes(item.status);
      }
  ...
  const handleToggleModifierGroup = (groupId) => {
    ...
    if (currentIds.includes(groupId)) {
      // Bỏ chọn
      setFormData({ ...formData, modifier_group_ids: currentIds.filter(id => id !== groupId) });
    } else {
      // Chọn thêm
      setFormData({ ...formData, modifier_group_ids: [...currentIds, groupId] });
    }
  };
  ...
  const handleSave = (e) => {
    e.preventDefault();
    if (formData.price < 0) { toast.error("Giá tiền không được phép âm!"); return; }
  ...
  return (
    <>
      <div className="admin-header">
        ...
        <button className="btn-primary" onClick={handleOpenCreate}>+ Thêm Món Mới</button>
      </div>
      ...
      <div className="filters-bar">
        <div className="search-box">
          ...
          <input type="text" placeholder="Tìm kiếm món ăn..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        </div>
          ...
          currentItems.map(item => (
            <div className="menu-admin-card" key={item.id}>
              <div className="menu-card-image" style={{ position: 'relative' }}>
                {/* Badge Recommended */}
                {item.is_recommended && (
                    <span style={{position:'absolute', top:10, right:10, background:'#f1c40f', color:'#fff', padding:'2px 8px', borderRadius:10, fontSize:12, fontWeight:'bold', zIndex:2, boxShadow:'0 2px 4px rgba(0,0,0,0.2)'}}>
                    ★ Chef's Choice
                    </span>
                )}
                {getPrimaryImage(item) ? <img src={getPrimaryImage(item)} alt={item.name} /> : <span style={{fontSize: 40}}>🍽️</span>}
                </div>
              ...
              <div className="menu-card-body">
                ...
                <p className="menu-card-category">{getCategoryName(item.category_id)}</p>
                ...
                <div className="menu-card-actions">
                  <button className="btn-icon" title="Sửa" onClick={() => handleOpenEdit(item)}>✎</button>
                  <button className="btn-icon" title="Nhân bản" onClick={() => handleDuplicate(item)}>📋</button>
                  <button className="btn-icon danger" title="Xóa" onClick={() => handleDelete(item.id)}>🗑️</button>
                </div>
              </div>
            </div>
          ))
        ) : (
          <div style={{gridColumn: '1 / -1', textAlign: 'center', padding: 40, color: '#999'}}>Không tìm thấy món ăn nào.</div>
        )}
      </div>
      ...
      {totalPages > 1 && (
        <div className="pagination">
          <button className="page-btn" disabled={currentPage === 1} onClick={() => setCurrentPage(prev => prev - 1)}>← Trang trước</button>
          <span className="page-info">Trang {currentPage} / {totalPages}</span>
          <button className="page-btn" disabled={currentPage === totalPages} onClick={() => setCurrentPage(prev => prev + 1)}>Trang sau →</button>
        </div>
      )}
      ...
      {showModal && (
        <div className="modal-overlay" onClick={() => setShowModal(false)}>
          <div className="modal-content" onClick={e => e.stopPropagation()} style={{ padding: 0, width: 600, maxWidth: '95%' }}>
            {/* Header */}
            <div style={{padding: '20px 30px', borderBottom: '1px solid #eee'}}>
              <h2 style={{margin:0}}>{isEditMode ? 'Chỉnh Sửa Món' : 'Thêm Món Mới'}</h2>
            </div>
            ...
            <div style={{display: 'flex', borderBottom: '1px solid #eee', background: '#f9f9f9'}}>
              {['info', 'photos', 'modifiers'].map(tab => (
                ...
                >
                  ...
                  {tab === 'modifiers' && `Topping / Tùy Chọn (${formData.modifier_group_ids.length})`}
                </button>
              ))}
            </div>
            ...
            <form onSubmit={handleSave}>
              <div style={{padding: 30, maxHeight: '60vh', overflowY: 'auto'}}>
                ...
                {activeTab === 'photos' && (
                  <div>
                    <div style={{display:'flex', gap:10, marginBottom: 15}}>
                      ...
                      />
                      <button type="button" className="btn-secondary" onClick={handleAddPhoto}>Thêm</button>
                    </div>

                    <div style={{display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(100px, 1fr))', gap:10}}>
                      {formData.photos.map(photo => (
                        <div key={photo.id} style={{position:'relative', border: photo.is_primary ? '2px solid var(--primary)' : '1px solid #ddd', borderRadius: 8, overflow:'hidden', height: 100}}>
                          <img src={photo.url} alt="Food" style={{width:'100%', height:'100%', objectFit:'cover'}} />
                          <div style={{position:'absolute', bottom:0, left:0, right:0, background:'rgba(0,0,0,0.6)', display:'flex', justifyContent:'center', padding: 5}}>
                            <button 
                              type="button" 
                              title="Đặt làm ảnh chính"
                              onClick={() => handleSetPrimaryPhoto(photo.id)}
                              style={{background:'transparent', border:'none', color: photo.is_primary ? '#f1c40f' : '#fff', cursor:'pointer', fontSize: 16}}
                            >★</button>
                            <button 
                              type="button" 
                              title="Xóa"
                              onClick={() => handleRemovePhoto(photo.id)}
                              style={{background:'transparent', border:'none', color:'#e74c3c', cursor:'pointer', fontSize: 16, marginLeft: 10}}
                            >🗑️</button>
                          </div>
                        </div>
                      ))}
                      {formData.photos.length === 0 && <p style={{gridColumn:'1/-1', color:'#999', textAlign:'center'}}>Chưa có ảnh nào.</p>}
                    </div>
                  </div>
                )}
                ...
                {activeTab === 'modifiers' && (
                  <div>
                    <p style={{marginBottom: 15, color:'#666'}}>Chọn các nhóm tùy chọn áp dụng cho món này:</p>
                    <div style={{display:'flex', flexDirection:'column', gap: 10}}>
                      {modifierGroups.length > 0 ? modifierGroups.map(group => (
                        <div key={group.id} style={{padding: 10, border:'1px solid #eee', borderRadius: 6, display:'flex', alignItems:'center'}}>
                          <input 
                            type="checkbox" 
                            id={`mod-${group.id}`}
                          ...
                          />
                          <label htmlFor={`mod-${group.id}`} style={{cursor:'pointer', flex: 1, fontWeight: 500}}>
                            {group.name}
                          </label>
                        </div>
                      )) : (
                        <p style={{color:'#999'}}>Chưa có nhóm Modifier nào. Hãy sang trang Modifiers để tạo.</p>
                      )}
                    </div>
                  </div>
                )}
              ...
              {/* Footer Buttons */}
              <div style={{padding: '20px 30px', borderTop: '1px solid #eee', display: 'flex', gap: 10, justifyContent: 'flex-end', background: '#fff'}}>

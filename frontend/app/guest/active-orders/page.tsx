...
import { RefreshCw, Clock, ChefHat, Bell, CheckCircle, CreditCard, Timer } from "lucide-react"
...
const statusConfig = {
  ...
  ready: { label: "Sẵn sàng", color: "bg-success text-success-foreground", icon: Bell },
  served: { label: "Đã phục vụ", color: "bg-accent text-accent-foreground", icon: CheckCircle },
  paid: { label: "Đã thanh toán", color: "bg-muted text-muted-foreground", icon: CreditCard },
}
...
export default function ActiveOrdersPage() {
  ...
  const fetchOrders = () => {
    ...
    const activeOrders = storedOrders.filter((order: Order) => order.status !== "paid")
  ...
  return (
    <div className="min-h-screen bg-background pb-20">
      <header className="sticky top-0 z-50 flex items-center justify-between border-b border-border bg-card px-4 py-3">
        <h1 className="text-lg font-bold text-card-foreground">Đơn hàng của bạn</h1>
      ...
      <main className="p-4">
        {orders.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-24 text-center">
            ...
            <p className="mt-2 text-muted-foreground">Đặt món ngay để bắt đầu</p>
                    ...
                    <div className="mb-3 space-y-1">
                      {order.items.slice(0, 3).map((item, index) => (
                        <div key={index} className="flex justify-between text-sm">
                          ...
                          <span className="text-muted-foreground">{formatPrice(item.totalPrice * item.quantity)}</span>
                        </div>
                      ))}
                      {order.items.length > 3 && (
                        <p className="text-sm text-muted-foreground">+{order.items.length - 3} món khác</p>
                      )}
                    </div>

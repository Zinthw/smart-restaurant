...
export default function WaiterLoginPage() {
  ...
  const handleGoogleCallback = async (response: any) => {
    ...
    try {
      ...
      if (data.user.role !== "waiter" && data.user.role !== "admin" && data.user.role !== "super_admin") {
        throw new Error("Bạn không có quyền truy cập trang này");
      }
      
      localStorage.setItem("waiterToken", data.token);
      localStorage.setItem("waiterName", data.user.name || data.user.email.split("@")[0]);
      ...
      <div className="w-full max-w-sm">
        ...
        <div className="rounded-xl border border-border bg-card p-6 shadow-sm">
          ...
          <div className="mt-6 rounded-lg bg-muted p-3 text-center text-sm text-muted-foreground">
            ...
            <p>Email: waiter@restaurant.com</p>
            <p>Mật khẩu: 123456</p>
          </div>
        </div>
      </div>
